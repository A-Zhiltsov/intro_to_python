...
def create_file(file_name):
    with open(file_name, "w",encoding='utf-8', newline='') as file:
        f_w = DictWriter(file, fieldnames=['Имя', 'Фамилия', 'Телефон'])
        f_w.writeheader()
...
def copy_data(file_name, sec_file_name):
    res1 = read_file(file_name)
    res2 = read_file(sec_file_name)
    search = int(input('Введите номер строки для копирования: '))
    res2.append(res1[search-1])
    standart_write(sec_file_name, res2)

    

file_name = 'phone.csv'
sec_file_name = 'phone2.csv'
    ...
    while True:
        ...
        if command == 'q':
            ...
            if not exists(file_name):
                print('файл отсутствует, пожалуйста создайте файл')
                continue
            if not exists(sec_file_name):
                print('файл отсутствует, пожалуйста создайте файл')
                continue
            copy_data(file_name, sec_file_name)
